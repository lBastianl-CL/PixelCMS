<?php
	...
	function friendListHome()
	{
		...
		if($infoRelationsNum == '0'){
			...
			'.userHome('username').' , je hebt nog geen vrienden hier!
			</div>
			';
			}else{
			if($infoRelationsNum == '1'){
				...
				$infoNumtext = "".userHome('username')." heeft nog <b>" . $infoRelationsNum . "</b> andere vriend(en) in deze categorie";
			}
			echo '
			<div class="friend_1"">
			<table>
		...
		if($infoRelationsNum == '0'){
			...
			<td>
			<div class="circle_friend">
			<div class="friend_head" style="background: url(https://habbo.com/habbo-imaging/avatarimage?figure='. filter($infoFriends['look']) .'&head_direction=2&action=wav&headonly=1)">
			</div>
			</div>
			</td>
			<td>
