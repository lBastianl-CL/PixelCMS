<?php
	...
	*/
	class Html 
	{
		...
		{
			global $dbh;
			$ipBan = $dbh->prepare("SELECT type,ban_expire FROM bans WHERE type = 'ip' && value = :ip");
			$ipBan->bindParam(':ip', $ip);
			$ipBan->execute();
			if ($ipBan->RowCount() === 1)
			{
				$queryIp = $dbh->prepare("SELECT type,ban_expire FROM bans WHERE type = 'ip' && value = :ip");
			...
			{
				$userBan = $dbh->prepare("SELECT type,ban_expire FROM bans WHERE type = 'user' && value = :username");
