...
<br><br>
	...
	<?php
			...
			{
				$getRanks = $dbh->prepare("SELECT * FROM permissions WHERE >=4 AND id <= 10   ORDER BY id DESC");
			}
			else
			{
				$getRanks = $dbh->prepare("SELECT id,name,badgeid FROM ranks WHERE id >=4 AND id <= 10 ORDER BY id DESC");
			}
			$getRanks->execute();
			...
			{	
				...
				if ($getMembers->RowCount() > 0)
				{
					...
					echo '
					<a href="/home/'.$username.'" style=" color: #fff; "><div style="pointer;float: left;border: 1px solid #e6e6e6;background: url('. H. $config['skin'].'/assets/images/alfapromo.png);padding-top: 20px;border-radius: 5px;width: 570px;margin-bottom: 10px;margin-left: 5px;margin-right: 5px;line-height: 30px;">
					<div id="column" style="border: 2px dotted rgba(0, 0, 0, 0.2);margin-top: -10px;margin-left: 10px;margin-right: 10px;margin-bottom: 10px;float: left;height:95px;width: 100px;border-radius: 555px;-moz-border-radius: 555px;-webkit-border-radius: 555px;background:url('.$config['HabboImg'].'/habbo-imaging/avatarimage?figure='.$look.'&head_direction=3&amp;action=wav&size=l) no-repeat rgba(255, 255, 255, 0.25);background-position: 60% 27%;"></div>
