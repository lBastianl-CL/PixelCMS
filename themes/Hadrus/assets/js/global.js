...
function changeAvatar(value, target) {  
	target = $(target);
	if(value.length != 0){
		target.css("background-image", "url(https://www.habbo.com.br/habbo-imaging/avatarimage?&user=" + value + "&action=std&direction=4&head_direction=3&img_format=png&gesture=spk&headonly=0&size=b");
	}
	else{
		target.css("background-image", "url(assets/image/ghost.png");  
	}
}
