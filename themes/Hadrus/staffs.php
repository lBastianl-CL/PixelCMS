...
<?php
			...
			{
				$getRanks = $dbh->prepare("SELECT * FROM permissions WHERE id >=9 AND id <= 13  ORDER BY id DESC");
			}
			else
			{
				$getRanks = $dbh->prepare("SELECT id,name,badgeid FROM ranks WHERE id >=9 AND id <= 13  ORDER BY id DESC");
			}
			$getRanks->execute();
            ...
            <div class="habbo-box content">
				...
				if ($getMembers->RowCount() > 0)
				{
					...
					echo '
					<a href="/home/'.$username.'" style=" color: #fff; "><div style="pointer;float: left;border: 1px solid #e6e6e6;background: url('. H. $config['skin'].'/assets/images/staffpromo.png);padding-top: 20px;border-radius: 5px;width: 610px;margin-bottom: 10px;margin-left: 5px;margin-right: 5px;line-height: 30px;">
